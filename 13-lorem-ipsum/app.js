...
const article = document.querySelector(".lorem-content");

// console.log(amount.value);
form.addEventListener("submit", function(e) {
    e.preventDefault();
    const random = Math.floor(Math.random() * textLength);
    randomPara(parseInt(amount.value), text, random);
});

function randomPara(amount, text, random) {
    let result = "";
    if(isNaN(amount) || amount < 0 || amount > 9  ) {
        result = `<p class="text">${text[random]}</p>`;
    } else {
